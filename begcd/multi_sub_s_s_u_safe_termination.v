...
Require Import simu.
Import simu.simu_m.
Require Import multi_sub_s_s_u_prg multi_sub_s_s_u_termination multi_sub_s_s_u_triple.
...
Lemma safe_termination_multi_sub_s_s_u z x y d L
  rz rx rk ry a0 a1 a2 a3 a4 ret rX rY :
  uniq(z, x, y) ->
  ...
  (multi_sub_s_s_u rk rz rx ry a0 a1 a2 a3 a4 ret rX rY).
Proof.
move=> Hvars Hregs.
rewrite /safe_termination.
move=> s st h [s_st_h [HL L_231]].
case/(multi_sub_s_s_u_termination st h) : (Hregs) => sf Hsf.
Require Import multi_sub_s_s_u_triple.
...
repeat (split => //).
rewrite HL Z_of_nat_Zabs_nat //; exact/min_u2Z.
