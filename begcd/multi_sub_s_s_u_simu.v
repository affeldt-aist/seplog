...
Require Import simu.
Import simu_m.
Require Import multi_sub_s_s_u_prg multi_sub_s_s_u_triple.
...
(** z <- x - y, z, x signed, y unsigned *)

Lemma pfwd_sim_multi_sub_s_s_u_wo_overflow (z x y : assoc.l) d k
  ...
  multi_sub_s_s_u rk rz rx ry a0 a1 a2 a3 a4 ret X Y.
Proof.
...
move/multi_sub_s_s_u_triple : (Hregs).
    ...
    * apply mkSignMagn => //.
      - rewrite lz'_k Zsgn_Zmult ZsgnK.
        ...
        rewrite lSum_Z2ints; last first.
          ...
          case: y_fit => _ y_fit.
          apply/(ltZ_leZ_trans y_fit)/leZP; rewrite /Zbeta Zpower_2_le; ssromega.
        rewrite /= /ZIT.add geZ0_norm //; tauto.
      - move/syntax_m.seplog_m.semop_prop_m.exec_cmd0_inv : exec_pseudo.
        case/syntax_m.seplog_m.exec0_assign_inv => _ ->.
        repeat syntax_m.seplog_m.assert_m.expr_m.Store_upd.
        rewrite lSum_Z2ints geZ0_norm in Z'_x_y; last by tauto.
        + case: (Z_zerop (s2Z lz')) => lz'_neq0.
          ...
          have {Ha3}Ha3 : u2Z [a3]_st' = 0.
            have Htmp : `|u2Z [a3 ]_ st' * \B^k + \S_{ k } Z'| < \B^k.
              ...
              apply (Z.lt_le_trans _ (\B^(k - 1) + \B^(k - 1))).
              apply Z.add_le_lt_mono => //; first exact: ltZW.
              rewrite Z.abs_opp geZ0_norm; tauto.
...
have Hd_unchanged : forall v r, assoc.get v d = Some r ->
  disj (mint_regs r) (mips_frame.modified_regs (multi_sub_s_s_u rk rz rx ry a0 a1 a2 a3 a4 ret X Y)).
  ...
  by move/assoc.get_Some_in_cdom : Hvr.
    ...
    - suff -> : heap_mint mx0 st' h' = heap_mint mx0 st h.
      ...
      case/assoc.get_union_Some_inv : (x0_mx0).
        ...
        suff Htmp : (var_e%asm_expr rx |--> lx :: px :: nil ** int_e px |--> X_)%asm_assert st'
          (heap_mint (signed k rx) st' h').
          apply: monotony Htmp => ?; by apply mapstos_ext.
      ...
      case/assoc.get_union_Some_inv.
        ...
        apply heap.inclu_union_L; first by map_tac_m.Disj.
        apply heap.inclu_union_R; first by map_tac_m.Disj.
        apply heap.inclu_union_R; first by [].
        by apply heap.inclu_refl.
        rewrite size_Z2ints.
  ...
  + move=> t Ht x0 Hx0.
    ...
    * case/assoc.in_cdom_union_inv.
      - rewrite assoc.cdom_sing seq.mem_seq1.
        ...
        apply uniq_disj. rewrite [cat _ _]/=. by Uniq_uniq r0.
      - case/assoc.in_cdom_union_inv.
        * rewrite assoc.cdom_sing mem_seq1.
          move/eqP=> ?; subst t.
