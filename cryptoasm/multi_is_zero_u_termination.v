...
- move=> kext IH s0 Hext h.
  apply exists_while.
  + rewrite /=; apply/eqP; rewrite Z_S in Hext; omega.
  + apply exists_seq_P2 with (fun s => u2Z [k ]_ (fst s) - u2Z [ext ]_ (fst s) = Z_of_nat kext)%mips_expr.
    * exists_lwxs l_z H_l_z z_z H_z_z.
      ...
      - apply exists_movn_false_seq_P => //.
        ...
        repeat Reg_upd.
        rewrite Z_S in Hext.
        rewrite sext_Z2u // u2Z_add_Z2u //; first omega.
        move: (min_u2Z [k ]_ s0) (max_u2Z [k ]_ s0) (min_u2Z [ext ]_ s0) (max_u2Z [ext ]_ s0) => ? ? ? ?; omega.
