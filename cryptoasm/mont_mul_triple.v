...
(** printing `* %\ensuremath{\odot}% *)

(** Formal separation-logic triple that specifies the Montgomery
multiplication.  In the post-condition, observe that the %\ensuremath{k+1}th%#k+1
th# word of storage is provided by the register %\coqdocvar{C}%#C#. The
...
apply trans_eq with (\S_{next} X * \S_{nk} Y +
...
case: Hinv => K [Hinv1 Hinv2].
exists K; split; last by []. (* about X * Y < M *)
by rewrite /= {1}addnS /= Hinv1 addnS.
...
case: Hinv => K [Hinv1 Hinv2].
exists K; split; last by []. (* about X * Y < M *)
rewrite -Hinv1 store.utoZ_maddu // Hone umul_1 (@u2Z_zext 32) mulZDl addZA /Sum_hole //.
